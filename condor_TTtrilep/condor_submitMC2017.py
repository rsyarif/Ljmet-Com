...
# 	os.system('tar --exclude="src/LJMet/Com/.git" --exclude="src/.git" --exclude="tmp" --exclude="*.scram" -zcf '+tarfile+' '+relBase.split('/')[-1])
	print 'tar --exclude="src/LJMet/Com/.git" --exclude="src/.git" --exclude="tmp" --exclude="*.scram" -zcvf '+tarfile+' '+relBase.split('/')[-1]+'/src/*' #only tar on the level where LJMet is
	os.system('tar --exclude="src/LJMet/Com/.git" --exclude="src/.git" --exclude="tmp" --exclude="*.scram" -zcvf '+tarfile+' '+relBase.split('/')[-1]+'/src/*') #only tar on the level where LJMet is
	os.chdir(thisDir)

cTime=datetime.datetime.now()
date='%i_%i_%i'%(cTime.year,cTime.month,cTime.day)
# outdir = 'LJMet94x_3lepTT_2017datasets_'+date+'_rizki'
outdir = 'LJMet94x_3lepTT_2017datasets_'+date+'_rizki_TEST_DELETE_ME'

for sample in sampleList:
	accessor = 'cmsxrootd.fnal.gov'
	NewPDF = False
	if 'prime' in sample or 'TEST' in sample: NewPDF = True
	os.system('python condor_submit.py --useMC True --sample '+sample.split('.')[0]+' --fileList '+thisDir+'fileLists2017/'+sample+' --submit True --inputTar '+tarfile+' --outDir /eos/uscms/store/user/lpcljm/'+outdir+' --shift '+shift+' --accessor '+accessor+' --newpdf '+str(NewPDF))
